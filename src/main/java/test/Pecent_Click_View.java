...
public class Pecent_Click_View {
	public static void main(String[] args) throws IOException {
		...
		JavaPairRDD<String, Long> counts = df.toJavaRDD().mapToPair(new PairFunction<Row, String, Long>() {
			...
			public Tuple2<String, Long> call(Row row) throws Exception {
				...
				if (cate.length() == 0) {
					...
					if (click_or_view == false) {
						String key = "crawl_err_view";
						long value = 1;
						Tuple2<String, Long> rs1 = new Tuple2<String, Long>(key, value);
						return rs1;
					} else {
						String key = "crawl_err_click";
						long value = 1;
						Tuple2<String, Long> rs1 = new Tuple2<String, Long>(key, value);
						return rs1;
					}
				} else {
					int[] index = Arrays.stream(cate.split(" ")).mapToInt(Integer::parseInt).toArray();
					boolean click_or_view = row.getBoolean(2);
		...
		StructType schema = new StructType(
				new StructField[] { new StructField("noname", DataTypes.StringType, true, Metadata.empty()),
